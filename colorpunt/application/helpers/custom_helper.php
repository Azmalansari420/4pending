...
function check_color_number($p_id,$bet_type)
{
  if($bet_type==1)
  {
    if($p_id==1)
    {
      $return = [1,3,7,9,5]; //green
    }
    else if($p_id==2)
    {
      $return = [0,5]; // violet
  ...
  else
  {
    if($p_id==0)
    {
      $return = [3,2]; // red violet
    }
    else if($p_id==5)
    {
      $return = [1,2]; // red violet
    }
    else if(in_array($p_id,[1,3,7,9]))
    {
      $return = [1]; // green
    }
...
function currentSession($gameId){
  $ci =& get_instance();
  $todaydate = date('Y-m-d H:i:s');
  $games = $ci->crud->selectDataByMultipleWhere('game', array('status' => 1,"id"=>$gameId,));
