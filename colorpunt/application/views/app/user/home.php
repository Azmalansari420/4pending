	...
	<header class="header header-fixed" >
		<div class="header-content">
			...
			<div class="mid-content header-logo">
				<a href="javascript:void(0);" style="font-size: 22px;background: white;padding: 0 6px;border-radius: 10px;">
		...
		<div class="container overflow-hidden pt-0">
			...
			<div class="top-bar-box">
				<div class="row">
					...
					<div class="col-6 text-center mt-2">
						<a href="withdraw.php" class="btn btn-danger btn-sm" style="color:white;background: #d23838;border-radius: .53333rem;">Withdraw</a>
					</div>
					<div class="col-6 text-center mt-2">
						<a href="deposite.php" class="btn btn-success btn-sm" style="color:white;">Deposit</a>
					</div>
				</div>
			</div>
			<!-- Products -->
			<div class="dz-box">
				...
				<div class="row gx-3 gy-4 timeLeft">					
					...
					<div class="col-6 mt-0 add-box-class your-parent-container" style="text-align:end;">
			                <span class="count-title d-block">Time Remaining</span>
			                <div class="counter-nu fs-4 fw-bold mb-2" id="countdown">00:00</div>
			                <p class="session-time small text-muted mt-2" style="color: white;" id="sessionNumber">Loading...</p>
					</div>					
				</div>

				<div class="row gx-3 gy-4 mt-4 main-box" id="main-box">
...
<!-- win -->
<div id="resultModal" class="modal modal-winn">
    <div class="modal-con1tent modal-content-animation win-content" style="background-color:black;">
        <span class="close" onclick="closeModal()">&times;</span>


        <div class="" id="resultModalBody">
        	
        </div>
        
        <div class="" id="winResultModalBody">
        	Win
        </div>

        <div class="" id="loseResultModalBody">
        	Lose
        </div>
...
<?php include('include/footer.php'); ?>
 ...
 <script>
	...
	var stopBetSeconds = 0;
	var userId = "<?=$full_detail->id ?>";
	var isReload = 1;
...
// Change this value to 30, 60, 120, etc. as per your session duration


function updategameTimer() {
    const sessionDuration = 60; // Example session duration in seconds
    const stopBetSeconds = 10;  // Seconds before session ends to stop betting
    ...
    const sessionNumber = Math.floor(secondsToday / sessionDuration) + 1;
    session_id = sessionNumber;

    // How many seconds passed in current session
    const secondsPassed = secondsToday % sessionDuration;
    const secondsLeft = sessionDuration - secondsPassed;

    // Format mm:ss
    const mins = Math.floor(secondsLeft / 60).toString().padStart(2, '0');
    const secs = (secondsLeft % 60).toString().padStart(2, '0');

    const secsCheck = parseInt(secs);
    const minsheck = parseInt(mins);

    if (minsheck === 0 && secsCheck <= stopBetSeconds) {
        $("#main-box").addClass("disabled");
    } else {
        $("#main-box").removeClass("disabled");
    }

    if (minsheck === 0 && secsCheck === 1) {
        isReload = 0;
        sessionDetails();
    }

    // Display
    $("#sessionNumber").text(`${sessionNumber}`);
...
function loadCurrentSession() 
{
	...
	var oldGameId = localStorage.getItem('selectedGameId');
    
    if(!oldGameId)
    {
    	gameId = $(".category-btn.game-active").data("game_id");
    }
    else
    {
    	gameId = oldGameId;
		$(".category-btn").removeClass("game-active");
		$(`.category-btn[data-game_id="${gameId}"]`).addClass("game-active");
    }
    

    $.ajax({
        url: '<?= base_url("api/Game_session/get_current_session") ?>',
        type: 'POST',
        data: { game_id: gameId },
        dataType: 'json',
        success: function (response) {
            if (response.success) {
            	sessionDuration = response.duration_seconds;
            	stopBetSeconds = response.stop_bet_seconds;
                updategameTimer();
                ...
                // $("#countdown").text("Wait");
                // $(".session-time").text("Waiting...");
                // setTimeout(() => loadCurrentSession(gameId), 3000); // Retry
            }
        }
    });
}
...
{
	$.ajax({
        url: '<?= base_url("api/Game_session/get_old_session_detail") ?>',
        type: 'POST',
        data: { game_id: gameId, session_id:session_id,user_id:userId },
        dataType: 'json',
        success: function (response) {
        	console.log(response)
            if (response.success) {
            	var game_result = response.game_result;
            	if(isReload==0)
            	{
            		showresultModal();
            		setTimeout(function(){
            		...
            		$("#winResultModalBody, #loseResultModalBody").hide();
            		if(game_result.is_bet==1)
            		{
            			if(game_result.is_bet==1)
            			{	
            				$("#winResultModalBody").hide();
            			}
            			else
            			{
            				$("#loseResultModalBody").hide();
