...
$(document).ready(function () {
    ...
    // Inject the countdown UI into the DOM if it doesn't already exist
    function injectCountdownUI() {
        if ($("#countdown-box").length === 0) {
            const countdownHtml = `
                <div id="countdown-box" class="col-6 mt-0 add-box-class" style="text-align:end;">
                    <span class="count-title">Time Remaining</span>
                    <div class="counter-nu" id="countdown">00:00</div>
                    <p class="session-time">Loading...</p>
                </div>`;
            $(".your-parent-container").append(countdownHtml);
        }
    }

    function toISOStringLocal(dateStr) {
        return dateStr.replace(" ", "T");
    }

    function loadCurrentSession(gameId) {
        clearInterval(timerInterval);
        injectCountdownUI();

        console.log("Loading session for Game ID:", gameId);
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.success) {
                    $(".session-time").text(response.session_id);
                    startCountdown(
					    toISOStringLocal(response.session_start_date_time),
					    toISOStringLocal(response.session_stop_date_time),
					    toISOStringLocal(response.session_result_declare_date_time),
					    gameId
					);
                } else {
                    $("#countdown").text("Wait");
                    ...
                    setTimeout(() => {
                        loadCurrentSession(gameId); // Retry if session not found
                    }, 3000);
                }
            }
        });
    ...
    function startCountdown(sessionStartTime, sessionStopTime, resultDeclareTime, gameId) {
	    clearInterval(timerInterval);

	    const start = new Date(sessionStartTime).getTime();
	    const stop = new Date(sessionStopTime).getTime();
	    const declare = new Date(resultDeclareTime).getTime();
	    ...
	    function updateTimer() {
	        ...
	        if (now >= declare) {
	            $("#countdown").text("Wait");
	            clearInterval(timerInterval);
	            loadCurrentSession(gameId); // reload after result
	        } else if (now >= stop) {
	            $("#countdown").text("Wait");
	        } else if (now < start) {
	            $("#countdown").text("00:00"); // if not started yet
	        } else {
	            const diff = stop - now;
    ...
    $(".category-btn").click(function () {
        ...
        currentGameId = $(this).data("game_id");
        localStorage.setItem('selectedGameId', currentGameId); // Store selected game ID
        loadCurrentSession(currentGameId);  // Load current session based on selected game ID
    });

    // Load previously selected game session
    let storedGameId = localStorage.getItem('selectedGameId');
		if (storedGameId) {
		    currentGameId = storedGameId;
		    $(".category-btn").removeClass("game-active"); // remove all
		    $(`.category-btn[data-game_id="${storedGameId}"]`).addClass("game-active"); // set only correct one
		    loadCurrentSession(currentGameId);
		}
