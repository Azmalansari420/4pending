...
class Role extends Controller
{
    protected $arr_values = array(
                            ...
                            'table_data_page_url'=>'admin/role/table',
                            'table_data_pagination_limit'=>10,
                            'load_list_path'=>'admin/role/list',
                            /*load form path and update url*/
                            'add_form_path'=>'admin/role/form',
                            'addupdate_form'=>'admin_con/role/storeOrUpdate',
                            /*add page*/
                            'add_page_link'=>'role/add',
                            /*edit page*/
                            'edit_page_url'=>'admin/role/edit',
                            'view_page_url'=>'admin/role/view',
                            /*status*/
                            'status_url'=>'admin/role/update_status',
                            /*delete*/
                            'delete_single_url'=>'admin_con/role/delete_data',
                            'multiple_delete'=>'admin_con/role/multiple_delete_data',
                            'check_image'=>true,
   ...
   public function getTableData(Request $request)
    ...
    public function storeOrUpdate(Request $request)
    {
        ...
        // Form arrays coming from checkbox menu
        $access_count = $request->input('access_count', []);
        $main_access  = $request->input('main_access', []);
        $access       = [];  

        foreach ($access_count as $key => $value) 
        {
            $inner = $request->input("inner_access{$key}", []);
            $access[$key] = !empty($inner) ? $inner : [];
        }
        // Combine final JSON like CI code
        ...
        if (empty($id)) {
            ...
            /*update all user with same role*/
            DB::table('tbl_admin')
                ->where('role', $insert_id)
                ->update(['access' => $role_access]);
